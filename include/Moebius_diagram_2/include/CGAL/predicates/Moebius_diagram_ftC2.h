			    ...
			    const RT &rx, const RT &ry, const RT &rl, const RT &rm)
{
  ...
  const RT a_ = determinant (ql_, qx_, rl_, rx_);
  const RT b_ = determinant (ql_, qy_, rl_, ry_);
  const RT c_ = determinant (Q_, ql_, R_, rl_);
				...
				const RT &rx, const RT &ry, const RT &rl, const RT &rm)
{
  ...
  return CGAL::enum_cast<Oriented_side> (CGAL_NTS compare (_2 * (a_ * qx_ + b_ * qy_), ql_ * c_));
}
...
{  
  ...
  const RT Dx = determinant (Q_,qy_,ql_, R_,ry_,rl_, S_,sy_,sl_);
  const RT Dy = determinant (qx_,Q_,ql_, rx_,R_,rl_, sx_,S_,sl_);
  const RT Dz = determinant (qx_,qy_,Q_, rx_,ry_,R_, sx_,sy_,S_);
  const RT D = determinant (qx_,qy_,ql_, rx_,ry_,rl_, sx_,sy_,sl_);

  CGAL_assertion (CGAL_NTS sign (D) == CGAL::POSITIVE);

  CGAL::Bounded_side result =
    CGAL::enum_cast<Bounded_side> (CGAL_NTS compare (_4 * D * Dz, Dx * Dx + Dy * Dy));
				...
				const RT &sx, const RT &sy, const RT &sl, const RT &sm)
{
  ...
  const RT a_ = determinant (qx_, ql_, rx_, rl_);
  const RT b_ = determinant (qy_, ql_, ry_, rl_);
  //  const RT c_ = determinant (Q_, ql_, R_, rl_);

  return CGAL::enum_cast<Oriented_side> (CGAL_NTS compare (ql_ * (a_ * Dy - b_ * Dx),
						_2 * D * (qx_ * b_ - qy_ * a_)));
}

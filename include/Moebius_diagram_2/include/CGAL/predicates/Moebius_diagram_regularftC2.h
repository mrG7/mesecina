		      ...
		      const RT &sx, const RT &sy, const RT &sl, const RT &sm)
{
  ...
  return Orientation (sign_of_determinant (qx_,qy_,ql_,
					      rx_,ry_,rl_,
					      sx_,sy_,sl_));
}
			       ...
			       const RT &rx, const RT &ry, const RT &rl, const RT &rm)
{
  ...
  const Orientation oxy_pqr = Orientation (sign_of_determinant (qx_,qy_, rx_,ry_));
  if (oxy_pqr != ZERO) return oxy_pqr;

  const Orientation oyz_pqr = Orientation (sign_of_determinant (qy_,ql_, ry_,rl_));
  if (oyz_pqr != ZERO) return oyz_pqr;

  return Orientation (sign_of_determinant (qx_,ql_, rx_,rl_));
}
		     ...
		     const RT &tx, const RT &ty, const RT &tl, const RT &tm)
{
  ...
  return Oriented_side (- sign_of_determinant (px_, py_, pl_, pw_,
						  qx_, qy_, ql_, qw_,
						  rx_, ry_, rl_, rw_,
						  sx_, sy_, sl_, sw_));
}
		     ...
		     const RT &tx, const RT &ty, const RT &tl, const RT &tm)
{
  ...
  Sign cmp;
  cmp = sign_of_determinant (px_, py_, pw_,
				qx_, qy_, qw_,
				rx_, ry_, rw_);
  if (cmp != ZERO)
    return Oriented_side (cmp * sign_of_determinant (plx-rlx, ply-rly,
							qlx-rlx, qly-rly));

  cmp = sign_of_determinant (px_, pl_, pw_,
				qx_, ql_, qw_,
				rx_, rl_, rw_);
  if (cmp != ZERO)
    return Oriented_side (cmp * sign_of_determinant (plx-rlx, rl-pl,
							qlx-rlx, rl-ql));
  
  cmp = sign_of_determinant (py_, pl_, pw_,
				qy_, ql_, qw_,
				ry_, rl_, rw_);
  return Oriented_side (cmp * sign_of_determinant (ply-rly, rl-pl,
						      qly-rly, rl-ql));
}
		     ...
		     const RT &tx, const RT &ty, const RT &tl, const RT &tm)
{
  ...
  if (cmp != EQUAL)
    return Oriented_side (cmp * sign_of_determinant (px_, pw_, qx_, qw_));

  cmp = CGAL_NTS compare (ply, qly);
  if (cmp != EQUAL)
    return Oriented_side (cmp * sign_of_determinant (py_, pw_, qy_, qw_));

  cmp = CGAL_NTS compare (ql, pl);
  return Oriented_side (cmp * sign_of_determinant (pl_, pw_, ql_, qw_));
}

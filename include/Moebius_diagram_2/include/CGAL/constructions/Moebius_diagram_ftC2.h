...
/* 	    <<"("<<sx<<", "<<sy<<", "<<sl<<", "<<sm<<")\n"; */
  ...
  const FT Rx = determinant (Q_,qy_,ql_, R_,ry_,rl_, S_,sy_,sl_);
  const FT Ry = determinant (qx_,Q_,ql_, rx_,R_,rl_, sx_,S_,sl_);
  const FT R = _two * determinant (qx_,qy_,ql_, rx_,ry_,rl_, sx_,sy_,sl_);
		 ...
		 FT &x, FT &y)
{
  ...
  const FT dx = determinant (Q_, qy_, R_, ry_);
  const FT dy = determinant (qx_, Q_, rx_, R_);
  const FT d = _2 * pl * determinant (qx_, qy_, rx_, ry_);
			  ...
			  FT &x1, FT &y1, FT &x2, FT &y2)
{
  ...
  const FT a = determinant (qx_, ql_, rx_, rl_);
  const FT b = determinant (qy_, ql_, ry_, rl_);
  const FT c = determinant (ql_, Q_, rl_, R_);
		      ...
		      FT &x, FT &y)
{
  ...
  const FT den = _2 * pl * determinant (qx_, qy_, rx_, ry_);

  x = determinant (qm_, qy_, rm_, ry_) / den;
  y = determinant (qx_, qm_, rx_, rm_) / den;
}
#endif //0
#if 0

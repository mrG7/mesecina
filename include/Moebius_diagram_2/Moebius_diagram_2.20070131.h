...
class Moebius_cell_base_3 : public Cb
{
 ...
 public:
  ...
  { set_inter_edge (this->index (e1), this->index (e2), inter); }
...
class Moebius_vertex_base_3 : public Vb
{
    bool _present;
public:
	...
	typedef Moebius_vertex_base_3<Gt, Vb2>         Other;
    };

    Moebius_vertex_base_3 ()
	: Vb(), _present (false) {}
    Moebius_vertex_base_3 (const Point&p)
	: Vb (p), _present (false) {}
    Moebius_vertex_base_3 (const Point&p, Cell_handle c)
	: Vb (p, c), _present (false) {}
	  
    bool present() const { return _present; }
    void set_present (bool p = true) { _present = p; }
};
...
public:
  ...
  typedef Moebius_vertex_base_3<Regular_traits> Vb2;
  typedef Triangulation_vertex_base_with_info_3<int,Regular_traits,Vb2> Vb;
  ...
  typedef typename RT_3::Finite_vertices_iterator Finite_vertices_iterator;
...
public:
  ...
  template <class InputIterator>
    int init (const InputIterator &first, const InputIterator &last)
    {
      ...
      CGAL::spatial_sort (points.begin(), points.end(), _rt.geom_traits());
      if (points.empty()) return 0;
      typename std::vector<Point>::iterator p = points.begin();

      Vertex_handle v = _rt.insert (*p);
      while (++p != points.end()) {
          Vertex_handle v_ = _rt.insert (*p, v->cell());
          if (v_ != Vertex_handle ()) v = v_;
      }
      return _rt.number_of_vertices();
    }
 ...
 int init (int n)
  ...
  Finite_vertices_iterator finite_vertices_begin () const { 
    ...
    return rt().finite_vertices_begin ();
  }

  Finite_vertices_iterator finite_vertices_end () const { 
    CGAL_precondition (_built);
    return rt().finite_vertices_end ();
  }
  ...
  void build () {
    ...
    Finite_edges_iterator end = rt().finite_edges_end ();
    for (; i != end; ++i) {
	if (inter_edge (*i) == 0) continue;
	i->first->vertex(i->second)->set_present(true);
	i->first->vertex(i->third)->set_present(true);
    }
		
    _built = true;
  }

  Vertex_handle locate (const Bare_point &p, Vertex_handle v = Vertex_handle()) const
  {
      	return rt().nearest_power_vertex (p,
		v == Vertex_handle() ? Cell_handle() : v->cell());
  }
 ...
 private:
  ...
  CGAL::Orientation orientation (const Vertex_handle &p, 
    ...
    { 
      ...
      return orientation (p->point().point(), q->point().point(), r->point().point());
    }
...
return s;
  ...
  CGAL::Bounded_side side_of_vertex (const Cell_handle &c) const
  {
      if (rt().is_infinite(c)) return CGAL::ON_UNBOUNDED_SIDE;
      return side_of_vertex (c, c->vertex(0), c->vertex(1));
  }
  ...
  void set_inter_facet (const Facet &f, int n) {
    ...
    if (dimension () > 2)
      f.first->neighbor(f.second)->set_inter_facet (rt().tds().mirror_index(f.first, f.second), n);
  }
